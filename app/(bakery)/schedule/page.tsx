...
function QuotaSheet({
  ...
  const handleRemove = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/production-schedule/${item.id}/${weekday}`, {
        method: 'DELETE',
        credentials: 'include',
      });
...
function DailySheet({
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/production-schedule/overrides`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
  ...
  const handleRemove = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/production-schedule/overrides/${item.id}/${dateStr}`, {
        method: 'DELETE',
        credentials: 'include',
      });
...
export default function SchedulePage() {
  ...
  const fetchOverridesForDate = useCallback(async (dateStr: string) => {
    try {
      const res = await fetch(`/api/production-schedule/overrides?date=${dateStr}`, { credentials: 'include' });
      if (!res.ok) return;
  ...
  const fetchAllUpcomingOverrides = useCallback(async (dates: string[]) => {
    ...
    try {
      const results = await Promise.all(
        dates.map(d =>
          fetch(`/api/production-schedule/overrides?date=${d}`, { credentials: 'include' })
            .then(r => r.ok ? r.json() : [])
        )
      );
