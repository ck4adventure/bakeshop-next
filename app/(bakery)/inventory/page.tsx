  ...
  const handleConfirmAdjust = async (item: InventoryItem, delta: number, note: string) => {
    ...
    try {
      const res = await fetch(`/api/inventory/adjust`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ itemId: item.itemId, delta: delta, note }),
      });
      if (!res.ok) {
    ...
    <div className="min-h-screen bg-background">
      ...
      {toast && (
        <div
          role="status"
          aria-live="polite"
          className="fixed top-4 left-1/2 -translate-x-1/2 bg-foreground text-background px-5 py-2.5 rounded-full text-sm font-medium z-200 shadow-lg whitespace-nowrap"
        >
          ✓ {toast}
        </div>
      )}
