...
const fakeSchedule = { id: 1, itemId: 1, weekday: 'Monday', quantity: 24 }
...
describe('POST /api/production-schedule', () => {
  ...
  it('returns 401 when not authenticated', async () => {
    mockGetServerSession.mockResolvedValue(null)
    const res = await POST(makePostRequest({ itemId: 1, weekday: 'Monday', quantity: 24 }))
    expect(res.status).toBe(401)
  })
  ...
  it('upserts schedule and returns 201', async () => {
    ...
    expect(mockPrisma.productionSchedule.upsert).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { itemId_weekday: { itemId: 1, weekday: 'Monday' } },
        update: { quantity: 24 },
        create: { itemId: 1, weekday: 'Monday', quantity: 24 },
      })
    )
  })
})
...
describe('DELETE /api/production-schedule/[itemId]/[weekday]', () => {
  ...
  it('returns 401 when not authenticated', async () => {
    mockGetServerSession.mockResolvedValue(null)
    const res = await DELETE(new Request('http://localhost'), makeDeleteParams('1', 'Monday'))
    expect(res.status).toBe(401)
  })
